...
import type { Symptom, Protocol } from '@/types/database'
...
export function useCheckIn(protocolId?: string) {
  ...
  const fetchData = async () => {
    try {
      ...
      if (protocolData) {
        const peptideData = protocolData.peptide as any
        const peptide = peptideData || { id: '', name: 'Unknown', dosage_amount: '', dosage_unit: '' }

        const transformedProtocol: Protocol = {
          id: protocolData.id,
          peptide_id: protocolData.peptide_id,
          clinician_id: '', // Not needed for check-in
          patient_id: user.id,
